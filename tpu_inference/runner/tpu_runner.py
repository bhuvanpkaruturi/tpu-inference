...
from tpu_inference.layers.common.attention_metadata import (
    ...
    pcp_token_layout, round_up_pcp_cache_pages)
...
class TPUModelRunner(KVConnectorModelRunnerMixin, LoRAModelRunnerMixin):
    ...
    def _init_inputs(self) -> None:
        ...
        self.attn_max_num_seqs = self.max_num_reqs * (
            2 if self.vllm_config.sharding_config.prefill_cp_size > 1 else 1)
        ...
        _pcp_pad = self.vllm_config.sharding_config.prefill_cp_size
        if _pcp_pad > 1:
            _worst = (scheduler_config.max_num_batched_tokens +
                      2 * _pcp_pad * scheduler_config.max_num_seqs)
        ...
        _pcp = self.vllm_config.sharding_config.prefill_cp_size
        _max_seqs = scheduler_config.max_num_seqs
        if _pcp > 1 and _max_seqs > 1:
    ...
    def _prepare_inputs(self, scheduler_output: "VllmSchedulerOutput"):
        ...
        if pcp_size > 1:
            ...
            # Announce genuinely concurrent PCP batches. Without this there is
            # no way to tell a multi-request build from one the scheduler only
            # ever fed a single request at a time -- which looks identical in
            # every benchmark number.
            # Bounded rather than one-shot: correlating an intermittent output
            # difference with the batch that produced it needs every batch, and
            # the first one is rarely the interesting one. Capped so a long
            # serving run cannot flood the log.
            _seen = getattr(self, "_pcp_multireq_seen", 0)
            if num_pcp_reqs > 1 and _seen < 200:
                self._pcp_multireq_seen = _seen + 1
                logger.info(
                    "PCP multi-request batch #%d: %d requests, "
                    "scheduled tokens %s, computed %s", _seen, num_pcp_reqs,
                    counts, computed)
            two_p = 2 * pcp_size
            ...
            src_off = np.concatenate([[0], np.cumsum(counts)[:-1]]).astype(
                np.int64) if counts else np.zeros(0, np.int64)
            ...
            for i in range(num_pcp_reqs):
                c_i = chunk[i]
                j = np.arange(c_i)
                kv_base = two_p * int(np.sum(chunk[:i]))
                for h in (0, 1):
            ...
            for i in range(padded_pcp_reqs):
                cu_row[2 * i + 1] = off[i] + chunk[i]
                cu_row[2 * i + 2] = off[i] + 2 * chunk[i]
            cu_row[n_layout_seqs + 1:] = cu_row[n_layout_seqs]
            ...
            for rank in range(pcp_size):
                pcp_cu_np[rank] = cu_row
                for i in range(padded_pcp_reqs):
                    pcp_qpos_np[rank, 2 * i] = rank * chunk[i]
                    pcp_qpos_np[rank, 2 * i + 1] = (two_p - 1 -
                                                    rank) * chunk[i]

            # Base of each request's block in the request-major K/V buffer.
            kv_new_starts_np = np.zeros(n_off, np.int32)
            kv_new_starts_np[:n_layout_seqs] = np.repeat(
                np.asarray(
                    [two_p * int(np.sum(chunk[:i]))
                     for i in range(padded_pcp_reqs)], np.int32), 2)
            ...
            pcp_metadata = PCPMetadata(
                ...
                kv_new_starts=device_array(
                    self.mesh, kv_new_starts_np, sharding=repl)
                if multi else None,
                kv_token_order=device_array(
                    self.mesh, kv_order, sharding=repl) if multi else None,
        ...
        def build_block_table_host(kv_cache_gid: int) -> None:
            ...
            if pcp_size > 1:
                ...
                # Descending, because the expansion is in place: request i's
                # row lives at i and must be read before row 2i overwrites it.
                for i in range(num_req_per_dp_rank[0] - 1, -1, -1):
                    row = block_tables_view[i].copy()
                    block_tables_view[2 * i] = row
                    block_tables_view[2 * i + 1] = row
