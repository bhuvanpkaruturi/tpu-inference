...
class CompilationManager:
    ...
    def _precompile_backbone_helper(self,
        ...
        if pcp_size > 1:
            ...
            if pcp_num_reqs > 1:
                two_p = 2 * pcp_size
                chunk = num_tokens // (pcp_size * 2 * pcp_num_reqs)
                assert chunk >= 1, (
                    f"{num_tokens=} cannot hold {pcp_num_reqs} PCP requests; "
                    "this bucket should have been skipped")
                for i in range(pcp_num_reqs):
                    off = 2 * chunk * i
                    cu_np[:, 2 * i + 1] = off + chunk
                    cu_np[:, 2 * i + 2] = off + 2 * chunk
                    kv_starts_np[2 * i] = kv_starts_np[2 * i + 1] = (two_p *
                                                                     chunk * i)
                    for r in range(pcp_size):
                        qpos_np[r, 2 * i] = r * chunk
                        qpos_np[r, 2 * i + 1] = (two_p - 1 - r) * chunk
                # keepdims: the trailing slice is empty when the fused seq
                # count exactly fills the buffer, and a (pcp_size,) RHS does
                # not broadcast against (pcp_size, m) for any m but 2.
                cu_np[:, 2 * pcp_num_reqs + 1:] = (
                    cu_np[:, 2 * pcp_num_reqs:2 * pcp_num_reqs + 1])
