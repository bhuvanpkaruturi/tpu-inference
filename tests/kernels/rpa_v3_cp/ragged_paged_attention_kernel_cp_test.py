...
class RaggedPagedAttentionPcpTest(jtu.JaxTestCase):
    ...
    def _multireq_layout(self, P, reqs):
        ...
        Mirrors the layout `_prepare_inputs` builds (design §4.1/§4.2): request
        ...
        # start at kv_new_starts[i] (design §4.4).
        ...
        pages = [np.arange(i * pps, (i + 1) * pps, dtype=np.int32)
                 for i in range(R)]
        ...
        return dict(R=R, C=C, off=off, S=S, n=n, L=L, two_p=two_p,
                    kv_starts=kv_starts, kv_total=kv_total, pps=pps,
                    pages=pages, cu_q_lens=self._padcu(cu),
                    kv_lens=self._pad1(
    ...
    def _pi_multi(self, pages, pps):
        ...
        return jnp.array(
            np.pad(flat, (0, self.MAX_SEQ * pps - len(flat))), jnp.int32)
    ...
    def test_pcp_multirequest_fused_write(self, P):
        ...
        for i in range(R):
            ...
            per_req_kv.append(merge_kv(jnp.array(ki, dtype),
                                       jnp.array(vi, dtype)))
    ...
    def test_pcp_multirequest_current_phase_output(self, P):
        ...
        for i in range(R):
            ...
            e, _ = ref_ragged_paged_attention(
                jnp.array(qi, dtype), jnp.array(ki, dtype),
                jnp.array(vi, dtype), self._empty_cache(dtype),
                self._pad1([ni]), self._pi(cdiv(ni, self.PAGE)),
                self._padcu([0, ni]), jnp.array([0, 0, 1], jnp.int32))
            exp.append(e[:ni])
        k = jnp.array(kv_buf_k, dtype)
        v = jnp.array(kv_buf_v, dtype)
